...
//if is used to specify a block of code to be executed if the conditional statement right after the if keyword is true.
// else if specifies a new condition to test only if the first condition/ or previous condition is false. 
//else is placed before a block of code to be executed if all previous if, or else-if conditional statements were false. else does not have a conditional statement that must be true in order for the code to run. If the code hits else, the code in the curly brackets after the else keyword will run.
